...
class Type
{
	...
	public function __construct() {
		...
		if (empty($this->slug) || empty($this->name) || empty($this->singular_name)) {
			throw new \RuntimeException("Must set custom taxonomy 'slug', 'name', and 'singular_name' properties.");
		}
		...
		register_taxonomy($this->slug, $this->object_type, $this->getSettings());
	}
	...
	protected function getSettings() {
		...
		foreach(get_class_vars($this) as $var => $default) {
			...
			} else if ($var !== 'name' && $var !== 'singular_name' && $var !== 'slug' && $var !== 'object_type') {
				if (isset($this->$var)) {
